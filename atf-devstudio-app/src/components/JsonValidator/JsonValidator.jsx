// JsonValidator.jsx
// JSON Validator component with tabbed interface
...
const STORAGE_KEY = 'atf-dev-studio-json-validator';
const DEFAULT_TAB_TITLE = 'JSON Validator';
...
const JsonValidator = () => {
  ...
  const createValidator = useCallback(() => {
    ...
    setActiveTabId(newValidator.id);
    showStatusMessage(setStatusMessage, 'New JSON validator tab created', statusTimeoutRef);
  ...
  const deleteValidator = useCallback((id) => {
    ...
    if (id === activeTabId) {
      if (updatedValidators.length > 0) {
        ...
        setValidators([newValidator]);
        setActiveTabId(newValidator.id);
        showStatusMessage(setStatusMessage, 'New JSON validator tab created automatically', statusTimeoutRef);
        return;
      }
    } else {
      setValidators(updatedValidators);
    ...
    showStatusMessage(setStatusMessage, 'JSON validator tab closed', statusTimeoutRef);
  ...
  const clearAllValidators = useCallback(() => {
    ...
    if (window.confirm('Are you sure you want to close all JSON validator tabs? A new tab will be created automatically.')) {
  ...
  return (
    <div className={`json-validator-container ${dashboardDarkMode ? 'dark-mode' : ''} ${validatorStyle === 'modern' ? 'modern-style' : ''}`} ref={containerRef}>
      <div className="json-validator-toolbar">
        <div className="toolbar-left">
          <button
            className="action-button create-validator-button"
            onClick={createValidator}
            title="Create a new JSON validator tab"
          >
            + New Validator
          </button>
          ...
          {validators.length > 0 && (
            <button
              ...
              title="Close all JSON validator tabs (with confirmation)"
            >
              Close All Tabs
            </button>
          )}
      ...
      <div className="json-validator-area" onClick={() => {
        if (validators.length === 0) {
          createValidator();
        }
      }}>
        {activeValidator ? (
          ...
          <div className="empty-state" style={{ cursor: 'pointer' }}>
            ...
            <p className="hint">Validate, format, minify, and convert JSON to XML, YAML, or HTML forms.</p>
          </div>
        )}
      </div>
