...
// Test Data Generator component with tabbed interface

import { useState, useEffect, useRef, useCallback } from 'react';
import { useOutletContext } from 'react-router-dom';
import TestDataGeneratorTab from './TestDataGeneratorTab';
...
const STORAGE_KEY = 'atf-dev-studio-test-data-generator';
const DEFAULT_TAB_TITLE = 'Test Data Generator';
...
const TestDataGenerator = () => {
  ...
  const createGenerator = useCallback(() => {
    ...
    showStatusMessage(setStatusMessage, 'New test data generator tab created', statusTimeoutRef);
  ...
  const deleteGenerator = useCallback((id) => {
    ...
    showStatusMessage(setStatusMessage, 'Test data generator tab closed', statusTimeoutRef);
  ...
  const clearAllGenerators = useCallback(() => {
    ...
    if (window.confirm('Are you sure you want to close all test data generator tabs? This action cannot be undone.')) {
      setGenerators([]);
      setActiveTabId(null);
      localStorage.removeItem(STORAGE_KEY);
      showStatusMessage(setStatusMessage, 'All test data generator tabs closed', statusTimeoutRef);
    }
  }, [generators.length]);
  ...
  return (
    <div className={`test-data-generator-container ${dashboardDarkMode ? 'dark-mode' : ''} ${generatorStyle === 'modern' ? 'modern-style' : ''}`} ref={containerRef}>
      <div className="test-data-generator-toolbar">
        <div className="toolbar-left">
          <button
            className="action-button create-generator-button"
            onClick={createGenerator}
            title="Create a new test data generator tab"
          >
            + New Generator
          </button>
          ...
          {generators.length > 0 && (
            <button
              ...
              title="Close all test data generator tabs (with confirmation)"
            >
              Close All Tabs
            </button>
          )}
      ...
      <div className="test-data-generator-area">
        {activeGenerator ? (
          <TestDataGeneratorTab
          ...
          <div className="empty-state">
            ...
            <p className="hint">Define your data schema in JSON format and generate test data in multiple formats.</p>
          </div>
        )}
      </div>
