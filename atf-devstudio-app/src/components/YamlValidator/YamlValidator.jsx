// YamlValidator.jsx
// YAML Validator component with tabbed interface

import { useState, useEffect, useRef, useCallback } from 'react';
import { useOutletContext } from 'react-router-dom';
import YamlValidatorTab from './YamlValidatorTab';
...
const STORAGE_KEY = 'atf-dev-studio-yaml-validator';
const DEFAULT_TAB_TITLE = 'YAML Validator';
...
const YamlValidator = () => {
  ...
  const createValidator = useCallback(() => {
    ...
    setActiveTabId(newValidator.id);
    showStatusMessage(setStatusMessage, 'New YAML validator tab created', statusTimeoutRef);
  ...
  const deleteValidator = useCallback((id) => {
    ...
    if (id === activeTabId) {
      if (updatedValidators.length > 0) {
        ...
        setValidators([newValidator]);
        setActiveTabId(newValidator.id);
        showStatusMessage(setStatusMessage, 'New YAML validator tab created automatically', statusTimeoutRef);
        return;
      }
    } else {
      setValidators(updatedValidators);
    ...
    showStatusMessage(setStatusMessage, 'YAML validator tab closed', statusTimeoutRef);
  ...
  const clearAllValidators = useCallback(() => {
    ...
    if (window.confirm('Are you sure you want to close all YAML validator tabs? A new tab will be created automatically.')) {
  ...
  return (
    <div className={`yaml-validator-container ${dashboardDarkMode ? 'dark-mode' : ''} ${validatorStyle === 'modern' ? 'modern-style' : ''}`} ref={containerRef}>
      <div className="yaml-validator-toolbar">
        <div className="toolbar-left">
          <button
            className="action-button create-validator-button"
            onClick={createValidator}
            title="Create a new YAML validator tab"
          >
            + New Validator
          </button>
          ...
          {validators.length > 0 && (
            <button
              ...
              title="Close all YAML validator tabs (with confirmation)"
            >
              Close All Tabs
            </button>
          )}
      ...
      <div className="yaml-validator-area" onClick={() => {
        if (validators.length === 0) {
          createValidator();
        }
      }}>
        {activeValidator ? (
          ...
          <div className="empty-state" style={{ cursor: 'pointer' }}>
            ...
            <p className="hint">Validate, format, and convert YAML to JSON, XML, or HTML forms.</p>
          </div>
        )}
      </div>
